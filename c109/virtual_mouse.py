...
def countFingers(image, hand_landmarks, handNo=0):
	...
	if hand_landmarks:
		...
		finger_top_y=int((landmarks[8].y)*height)
		thumb_tip_x=int((landmarks[4].x)*width)
		thumb_tip_y=int((landmarks[4].y)*height)
		cv2.line(image,(finger_tip_x,finger_top_y),(thumb_tip_x,thumb_tip_y),(0,0,255),2)
		
		

		# Draw a CIRCLE on CENTER of the LINE between FINGER TIP and THUMB TIP
		centerx=int((finger_tip_x+thumb_tip_x)/2)
		centery=int((finger_top_y+thumb_tip_y)/2)
		cv2.circle(image,(centerx,centery),2,(255,255,255),2)
		

		# Calculate DISTANCE between FINGER TIP and THUMB TIP
		distance=math.sqrt(((finger_tip_x-thumb_tip_x)**2)+((finger_top_y-thumb_tip_y)**2))
		relativemousex=(centerx/width)*screen_width
		relativemousey=(centery/height)*screen_height
		mouse.position=(relativemousex,relativemousey)
